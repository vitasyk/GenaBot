from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton

def get_generator_control_kb(statuses: dict = None, exclude_correction: bool = False, exclude_stop_all: bool = False) -> InlineKeyboardMarkup:
    # Default to stopped if not provided
    s1 = statuses.get("GEN-1 (036)", "🔴") if statuses else "🔴"
    s2 = statuses.get("GEN-2 (003) WILSON", "🔴") if statuses else "🔴"
    
    buttons = [
        [
            InlineKeyboardButton(text="🟢 Старт GEN-1 (036)", callback_data="start_gen_1"),
            InlineKeyboardButton(text="🟡 Чергування GEN-1 (036)", callback_data="standby_gen_1")
        ],
        [
            InlineKeyboardButton(text="🟢 Старт GEN-2 (003) WILSON", callback_data="start_gen_2"),
            InlineKeyboardButton(text="🟡 Чергування GEN-2 (003) WILSON", callback_data="standby_gen_2")
        ]
    ]
    
    bottom_row = []
    
    if not exclude_stop_all:
        bottom_row.append(InlineKeyboardButton(text="🛑 Зупинити всі", callback_data="stop_all_gens"))
    
    if not exclude_correction:
        bottom_row.append(InlineKeyboardButton(text="🔧 Корегування", callback_data="correct_fuel_menu"))
    
    if bottom_row:
        buttons.append(bottom_row)
    return InlineKeyboardMarkup(inline_keyboard=buttons)
